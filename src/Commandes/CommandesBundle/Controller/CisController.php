...
use Commandes\CommandesBundle\Entity\Cis;
...
class CisController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $cis = $em->getRepository('CommandesBundle:Cis')->findAll();

        return $this->render('cis/index.html.twig', array(
            'cis' => $cis,
        ));
    }
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $nf = $em->getRepository('CommandesBundle:Cis')->idfact();
        $bl = new Cis();
        $form = $this->createForm('Commandes\CommandesBundle\Form\CisType', $bl);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('admin_cis_show', array('id' => $bl->getId()));
        }
        ...
        return $this->render('cis/new.html.twig', array(
            'bl' => $bl,
    ...
    /**
     * Finds and displays a ci entity.
     *
     */
    public function showAction(Cis $ci)
    {
        $deleteForm = $this->createDeleteForm($ci);

        return $this->render('cis/show.html.twig', array(
            'ci' => $ci,
    ...
    /**
     * Displays a form to edit an existing ci entity.
     *
     */
    public function editAction(Request $request, Cis $ci)
    {
        $deleteForm = $this->createDeleteForm($ci);
        $editForm = $this->createForm('Commandes\CommandesBundle\Form\CisType', $ci);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('admin_cis_edit', array('id' => $ci->getId()));
        }

        return $this->render('cis/edit.html.twig', array(
            'ci' => $ci,
     ...
     *
     */
    public function deleteAction(Request $request, Cis $ci)
    {
        $form = $this->createDeleteForm($ci);
        $form->handleRequest($request);
        ...
        return $this->redirectToRoute('admin_cis_index');
    }

    /**
     * Creates a form to delete a ci entity.
     *
     * @param Cis $ci The ci entity
     *
     * @return \Symfony\Component\Form\Form The form
     */
    private function createDeleteForm(Cis $ci)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('admin_cis_delete', array('id' => $ci->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
